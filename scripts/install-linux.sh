...
log() { printf '[bitshit] %s\n' "$*"; }
warn() { printf '[bitshit] %s\n' "$*" >&2; }
die() { printf '[bitshit] %s\n' "$*" >&2; exit 1; }
...
else die "Missing C++ compiler (c++, clang++, or g++)"; fi
...
has_metal() { [[ "$(uname -s)" == "Darwin" ]] && command -v xcrun >/dev/null 2>&1 && xcrun --find clang >/dev/null 2>&1; }
auto_backend() { if has_cuda; then echo cuda; elif has_rocm; then echo rocm; elif has_metal; then echo metal; else echo cpu; fi; }
...
if [[ -z "$BACKEND" ]]; then
  ...
  if [[ $ASSUME_YES -eq 1 || ! -t 0 ]]; then BACKEND="$DETECTED"
    ...
    read -r -p '> ' answer
    case "$answer" in 1|"") BACKEND="$DETECTED" ;; 2) BACKEND=cpu ;; 3) BACKEND=cuda ;; 4) BACKEND=rocm ;; 5) BACKEND=metal ;; *) die "Invalid selection" ;; esac
  fi
elif [[ "$BACKEND" == auto ]]; then BACKEND="$(auto_backend)"; fi

case "$BACKEND" in
  cuda) has_cuda || die "CUDA selected but both toolkit and driver/device are required." ;;
  rocm) has_rocm || die "ROCm selected but hipcc and rocminfo are required." ;;
  metal) has_metal || die "Metal selected but macOS with Xcode tools is required." ;;
esac
...
(
  cd "$SOURCE_DIR"
  if [[ -f Cargo.lock ]]; then cargo build --locked --profile "$PROFILE" -p cmd --bin "$TARGET_BIN"
  else warn "Cargo.lock is missing; generating a lockfile before the build"; cargo generate-lockfile; cargo build --profile "$PROFILE" -p cmd --bin "$TARGET_BIN"; fi
)
...
{"product":"bitshit","backend":"$BACKEND","profile":"$PROFILE","binary":"$INSTALL_DIR/$TARGET_BIN","source":"$SOURCE_DIR"}
EOF
