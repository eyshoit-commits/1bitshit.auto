...
if ($env:OS -ne 'Windows_NT') {
    Fail 'bootstrap-windows.ps1 ist nur fuer Windows. Verwende bootstrap-linux.sh.'
}

Say 'BitShit Windows Bootstrap startet.'
...
$ResolvedBackend = if ([string]::IsNullOrWhiteSpace($Backend)) { 'auto' } else { $Backend }
...
Say 'Windows Umgebung installiert.'
Say 'Falls Build Tools oder CUDA neu installiert wurden, schliesse dieses Fenster und oeffne PowerShell erneut.'
